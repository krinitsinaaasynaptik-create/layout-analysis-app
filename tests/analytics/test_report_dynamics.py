...
class ReportDynamicsTest(unittest.TestCase):
    ...
    @patch("app.report.latest_run", return_value=None)
    @patch("app.report.fetch_report_rows")
    def test_project_price_history_maps_legacy_single_house_rows(self, fetch_report_rows_mock, _latest_run_mock) -> None:
        fetch_report_rows_mock.return_value = {
            "developers": [
                {"id": "zhcom", "name": "Железно", "type": "competitor"},
            ],
            ...
            "objectiv_project_history_monthly": [
                {
                    ...
                    "project_name": "Дом Булычев",
                    "month_key": "2022-03",
                    "snapshot_date": "2022-03-31",
                    "avg_price_per_sqm": 160650.39,
                    "apartments_count": 1,
                },
                {
                    "developer_id": "zhcom",
                    ...
                    "project_name": "Дом Булычев",
                    "month_key": "2022-04",
                    "snapshot_date": "2022-04-30",
                    "avg_price_per_sqm": 139442.16,
                    "apartments_count": 4,
                },
            ],
            "layout_tags": [],
        ...
        history = report["project_price_history"]
        self.assertTrue(history["available"])
        self.assertEqual(history["selected_project_id"], "zhcom:дом булычев")
        self.assertEqual(history["selected_house_id"], "zhcom:дом булычев:дом 28")
        self.assertEqual([row["key"] for row in history["series"]], ["2022-03", "2022-04"])
        self.assertEqual([row["value"] for row in history["series"]], [160650.39, 139442.16])

    @patch("app.report.latest_run", return_value=None)
    @patch("app.report.fetch_report_rows")
    def test_project_price_history_matches_objectiv_house_name_to_site_house(self, fetch_report_rows_mock, _latest_run_mock) -> None:
        fetch_report_rows_mock.return_value = {
            "developers": [
                {"id": "zhcom", "name": "Железно", "type": "competitor"},
            ],
            ...
            "objectiv_project_history_monthly": [
                {
                    "developer_id": "zhcom",
                    "project_id": "objectiv:123",
                    "project_name": "Дом Булычев",
                    "house_id": "objectiv:456",
                    "house_name": "Дом Булычев, корпус 28",
                    "month_key": "2022-03",
                    "snapshot_date": "2022-03-31",
                    "avg_price_per_sqm": 160650.39,
                    "apartments_count": 1,
